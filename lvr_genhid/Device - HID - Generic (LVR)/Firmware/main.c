...
File Description:
 ...
 Change History:
  ...
  0.9	1/11/13		Updated previous Generic HID example for use with 
					Microchip Libraries for Applications v2012-10-15 
					and mcc18 v3.44.  

I created this example by adapting Microchip's USB example titled
Device - HID - Custom Demos (v2.9f). 
	...
	switch (usb_state)
    {
			...
			if(!HIDRxHandleBusy(USBOutHandle))		   			
                        {
                            ...
                            if (len > 0)
                            {
                                    ...
                                    {
                                            ToSendDataBuffer[count] = ReceivedDataBuffer[count];
                                    }
		...
		case 't':
		{
			if (!HIDTxHandleBusy(USBInHandle))		 
			{
				...
				USBInHandle = HIDTxPacket(HID_EP,(BYTE*)&ToSendDataBuffer[0],HID_INPUT_REPORT_BYTES);
...
void User_Get_Report_Handler(void)
{
		...
		if ((SetupPkt.W_Value.byte.HB) == 0x01)	
		{
			...
			{	
				...
				USBEP0SendRAMPtr((BYTE*)&hid_report_out[0],HID_INPUT_REPORT_BYTES,USB_EP0_INCLUDE_ZERO);
			}
		}
		...
		else if (SetupPkt.W_Value.byte.HB == 0x03)
		{
			...
			{	
				...
				USBEP0SendRAMPtr((BYTE*)&hid_report_feature[0],HID_FEATURE_REPORT_BYTES,USB_EP0_INCLUDE_ZERO);
			}
		}
	#endif	
		...
		switch (ReportSupported())
	    {
			...
			{	
				if (SetupPkt.W_Value.byte.LB == 0x00)				
				{
				    // Report ID = 0, supported
                                    ...
                                    outPipes[0].pDst.bRam = (BYTE*)OUTPUT_REPORT;
			...
			{	
				if (SetupPkt.W_Value.byte.LB == 0x00)		
				{
                                    ...
                                    outPipes[0].wCount.Val = SetupPkt.wLength;
                                    outPipes[0].pDst.bRam = (BYTE*)FEATURE_REPORT;
                                    outPipes[0].pFunc = SET_REPORT_PFUNC;
                                    outPipes[0].info.bits.busy = 1;
                                }
			break;
