...
/* Maximum length of a string read from the Configuration file (cyusb.conf) for the library. */
...
/*
   struct VPD
   Used to store information about the devices of interest listed in cyusb.conf
 */
struct VPD {
...
/* The following variables are used by the cyusb_linux application. */
       char		pidfile[MAX_FILEPATH_LENGTH];	/* Full path to the PID file specified in cyusb.conf */
       char		logfile[MAX_FILEPATH_LENGTH];	/* Full path to the LOG file specified in cyusb.conf */
