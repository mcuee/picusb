/********************************************************************
 ...
 Company:		Microchip Technology, Inc. modified by Jan Axelson
                jan (at) Lvr.com
...
********************************************************************
 ...
 Change History:
  ...
  0.9   8/8/2009    Original adaptation
  0.91  8/17/2009     Minor mods for simplicity

I created this example by adapting Microchip's example titled
USB Device - HID - Custom Demos > Generic HID Firmware. 

This file has the following changes to Microchip's main.c:
...
void User_Get_Report_Handler1(void)
void User_Set_Report_Handler1(void)

I left the rest unchanged though the example doesn't use all of the
routines and I tested the example only on the PICDEM FS USB DEMO BOARD.

See readme.txt for more information about this example.
...
{   
    ...
    if((USBDeviceState < CONFIGURED_STATE)||(USBSuspendControl==1)) return;
   	...
   	// Send and receive HID reports using the interrupt endpooint addresses.
...
void USBCBInitEP(void)
{
	...
	USBOutHandle = HIDRxPacket(HID_EP,(BYTE*)&hid_report_out,64);
}
 ...
 *         			at the control enpdpoint. If yes, extracts and uses the data.
 *
 * Note:            None
 *****************************************************************************/
...
void mySetReportHandler(void)
{
	...
	switch (MSB(SetupPkt.W_Value))
    {
		...
		{	
		    ...
		    {
				case 2: // Report ID 2; Output report using Control Transfer
					...
					hid_report_in[0]=1; //Input report, Report ID 1
					hid_report_out[0]=2; //Output report, Report ID 2
					for(count=1; count <= HID_OUTPUT_REPORT_BYTES - 1; count++)
    				{   										
		...
		case 0x03: // Feature report 
		{
    		...
    		switch(LSB(SetupPkt.W_Value))
		    {
				...
				hid_report_feature[0]=3;
				hid_report_feature1[0]=4;												
				for(count=1; count <= HID_OUTPUT_REPORT_BYTES - 1; count++)
    				{   										
						hid_report_feature1[count] = hid_report_feature[count] ;
    				}				
	...
	switch (usb_state)
    {
		case 'r':
		{
	    	...
	    	{		
				...
				len = USBHandleGetLength(USBOutHandle); 
	
				if (len > 0)
				{	
					// Data was received. Copy it to the send buffer for sending back to the host.							
					hid_report_in[0]=1;
					hid_report_out[0]=2;
					for (count=1; count <= HID_OUTPUT_REPORT_BYTES - 1; count++)
					{
		...
		case 't':
		{
			if (!HIDTxHandleBusy(USBInHandle))		 
			{
				...
				USBInHandle = HIDTxPacket(HID_EP,(BYTE*)&hid_report_in[0],HID_INPUT_REPORT_BYTES);
				
				//Re-arm the OUT endpoint to enable receiving new data.

    			USBOutHandle = HIDRxPacket(HID_EP,(BYTE*)&hid_report_out,HID_INPUT_REPORT_BYTES);
...
BYTE ReportSupported(void)
{
	...
	switch (MSB(SetupPkt.W_Value))
    {
		...
		case 0x03: // Feature report 
		{
    		switch(LSB(SetupPkt.W_Value))
		    {
				case 0x03: // Report ID 3
				{
					return 3;					
				}
				case 0x04: // Report ID 4
				{
					return 3;					
				}
				default:
...
/********************************************************************
 * Function:        void User_Get_Report_Handler1(void)
 *
 ...
 *                  returns STALL.
 *                  This code is similar to the code for GET_MAX_LUN in 
...
void User_Get_Report_Handler(void)
{
		...
		if ((SetupPkt.W_Value.byte.HB) == 0x01)	
		{
			...
			if (SetupPkt.W_Value.byte.LB == 0x01)		
			{	
				// Report ID = 1, supported
				hid_report_in[0]=1; //prefix to the input report ID: 1
				USBEP0SendRAMPtr((BYTE*)&hid_report_in[0],HID_INPUT_REPORT_BYTES,USB_EP0_INCLUDE_ZERO);
			}
		}
		...
		else if (SetupPkt.W_Value.byte.HB == 0x03)
		{
			...
			if (SetupPkt.W_Value.byte.LB == 0x04)		
			{	
				// Report ID = 4, supported
				hid_report_feature1[0]=4; //prefix to the feature report ID:4
				USBEP0SendRAMPtr((BYTE*)&hid_report_feature1[0],HID_FEATURE_REPORT_BYTES,USB_EP0_INCLUDE_ZERO);
			}
		}
	#endif	
 
}//end User_Get_Report_Handler1

/********************************************************************
 * Function:        void User_Set_Report_Handler1(void)
 *
 ...
 *                  This code is similar to the code for SET_LINE_CODING in 
...
void User_Set_Report_Handler(void)
{
		...
		switch (ReportSupported())
	    {
			case 0x02: // Output report 
			{	
				if (SetupPkt.W_Value.byte.LB == 0x02)				
				{
					// Report ID = 2, supported		
					hid_report_out[0]=2; //prefix to the Output report ID:2
					outPipes[0].wCount.Val = SetupPkt.wLength;
		            outPipes[0].pDst.bRam = (BYTE*)OUTPUT_REPORT2;
			...
			case 0x03: // Feature report 
			{	
				if (SetupPkt.W_Value.byte.LB == 0x03)		
				{
					// Report ID = 3, supported
					hid_report_feature[0]=3; //prefix to the feature report ID:3
					outPipes[0].wCount.Val = SetupPkt.wLength;
		            outPipes[0].pDst.bRam = (BYTE*)FEATURE_REPORT3;
		            outPipes[0].pFunc = SET_REPORT_PFUNC;
		            outPipes[0].info.bits.busy = 1;		
				}	
			break;
